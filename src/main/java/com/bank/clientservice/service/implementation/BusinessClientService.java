...
public class BusinessClientService implements ClientService<BusinessClientRequest, BusinessClientResponse> {
    ...
    public Mono<OperationResponse> save(BusinessClientRequest request) {
        return repository.save(mapper.toDocument(request))
                .map(businessClient -> {
                    ...
                    return new OperationResponse(Message.CREATED_SUCCESSFULLY, HttpStatus.CREATED);
                });
    }
                ...
                .flatMap(exists -> {
                    if (Boolean.TRUE.equals(exists)) {
                        return repository.save(document)
                                .map(savedDocument -> new OperationResponse(Message.UPDATED_SUCCESSFULLY, HttpStatus.ACCEPTED));
                    } else {
                        return Mono.just(new OperationResponse(Message.NOT_FOUND, HttpStatus.NOT_FOUND));
                    }
                })
    ...
    public Mono<OperationResponse> delete(String id) {
        return repository.deleteById(id)
                .then(Mono.fromCallable(() -> new OperationResponse(Message.DELETED_SUCCESSFULLY, HttpStatus.ACCEPTED)))
